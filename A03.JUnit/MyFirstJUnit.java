...
public class MyFirstJUnit {
	...
	assertEquals(0, list.size(), "New list is not empty");
        ...
        //if(list.size() != 4) {
          //  System.out.println("List size is not 4 after calling add 4 times");
            //return false;}
	assertEquals(4, list.size(), "List size isn't 4 after 4 additions");
        list.clear();
        //if(list.size() != 0) {
          //  System.out.println("List size is not 0 after clearing");
            //return false;}
	assertEquals(0, list.size(), "List size isn't 0 after clearing");
        //return true;
    }
    ...
    public void testInserting100Numbers() {
		...
		int a = list.get(i);
		int b = list.get(i+1);
		assertTrue(a < b, "Found non-ascending elements " + a +  " and " + b + " at indexes " + i + ", and " + (i+1) + " respectively.");
	...
	assertEquals("durian", threeWas, "Issue when removing from the end.");
	assertEquals("cherry", twoIs, "new final element is wrong.");
	...
	assertEquals("apple", zeroWas, "Issue when removing from the front.");
	assertEquals("banana", zeroIs, "new first element is wrong.");
	...
	assertEquals(20, list.size(), "Expected size 20 after 20 additions.");
	...
	list.add(21);
	assertEquals(21, list.size(), "Exepcted size 21 after 20 + 1 additions.");
